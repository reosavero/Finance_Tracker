...
// PayBillModal di-portal ke document.body agar tidak
// terpotong oleh overflow-hidden / transform parent
// =====================================================
const UpcomingBills = ({ bills = [], onPaySuccess }) => {
  ...
  return (
    <>
      {/* Widget container — TANPA overflow-hidden agar konten tidak terpotong */}
      <div className="brutal-card p-0 overflow-visible">
        {/* Header */}
        ...
        <div className={expanded ? '' : 'max-h-0 overflow-hidden'}>
          {sortedGroups.map(({ status, config, bills: groupBills }) => (
            <div key={status} className={`${config.cardBg} border-t-3 ${config.cardBorder}`}>
              {/* Group header */}
              <button
              ...
              <div className="px-5 pb-3 space-y-2">
                {groupBills.map((bill) => {
                  ...
                  return (
                    <div
                      key={bill.id}
                      className={`flex items-center gap-3 p-3 rounded-brutal border-2 bg-white border-navy/10 transition-all hover:border-navy/30 ${
                        isPaid ? 'opacity-50' : ''
                      }`}
                    >
                      {/* Day badge */}
      ...
      {/* 
        PayBillModal di-render via createPortal ke document.body
        agar TIDAK terpotong oleh overflow-hidden / CSS transform 
        dari parent container (animate-slide-up, overflow-hidden, dll.)
        Modal dengan position:fixed yang berada di dalam elemen 
        yang punya CSS transform akan di-positioning relatif 
        ke elemen tsb, bukan viewport — inilah yang menyebabkan 
        popup terpotong.
      */}
