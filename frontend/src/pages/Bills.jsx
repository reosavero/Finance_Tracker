...
import { HiOutlinePlus, HiOutlineTrash, HiOutlineCheck, HiOutlinePencilAlt } from 'react-icons/hi';
    ...
    <div className="space-y-6">
        ...
        <div className="brutal-card animate-slide-up">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
              className="input-brutal" placeholder="Nama (cth: Sewa Kos)" required disabled={submittingCreate} />
            <input type="number" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })}
              className="input-brutal" placeholder="Nominal (Rp)" min="1" required disabled={submittingCreate} />
            <input type="number" value={form.due_day} onChange={(e) => setForm({ ...form, due_day: e.target.value })}
              className="input-brutal" placeholder="Tgl jatuh tempo (1-31)" min="1" max="31" required disabled={submittingCreate} />
            <select value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })} className="select-brutal" disabled={submittingCreate}>
              <option value="">Kategori (opsional)</option>
              {categories.map((c) => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
            </select>
            <div className="sm:col-span-2">
              <button type="submit" disabled={submittingCreate} className="btn-brutal-secondary w-full sm:w-auto disabled:opacity-50">
      ...
      {loading ? (
        ...
        <div className="brutal-card text-center py-12">
          ...
          <button onClick={() => setShowForm(true)} className="btn-brutal mt-5 text-sm">Tambah Tagihan Pertama</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {bills.map((bill, i) => {
              ...
              <div key={bill.id} className="brutal-card-hover animate-slide-up" style={{ animationDelay: `${i * 60}ms` }}>
                <div className="flex items-start justify-between mb-3 gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-14 h-14 rounded-brutal border-3 border-navy bg-warning/20 flex items-center justify-center shrink-0">
                      <span className="text-navy font-bold text-lg">{bill.due_day}</span>
                    </div>
                    <div className="min-w-0">
                      <p className="font-bold text-navy text-lg truncate">{bill.name}</p>
                  ...
                  <span className={`text-xs px-3 py-1 rounded-full border-2 font-bold shrink-0 ${st.cls}`}>{st.label}</span>
                </div>
